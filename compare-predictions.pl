...
#if (scalar @files != 2) {
#    croak "Please give two files for comparison";
#}

## TODO: this is the same as in blend-predictions.pl -- move to a module!
my %predictions = ();
...
foreach my $key (@reference_keys) {
    foreach my $file (@files) {
        my $difference = $predictions{$reference_file}->{$key} - $predictions{$file}->{$key};
...
sub usage {
    ...
    print << 'END';
Compare two different predictions (MovieLens format).
(c) 2008 Zeno Gantner
