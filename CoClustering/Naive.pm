...
package CoClustering::Naive;

# Function prototypes:
sub contains_only_zeros ($);
sub static_training (@);
sub compute_global_average ($$);
...
sub static_training (@) {
	...
	my $global_average = compute_global_average($ratings, $confidence);
	...
	for (my $i = 0; $i < $m; $i++) {
		...
		if ($confidence_sum != 0) {
			...
			# This should not happen, such rows should be eliminated before the training.
		}
	...
	for (my $j = 0; $j < $n; $j++) {
		...
		if ($confidence_sum != 0) {
			...
			# TODO: This should not happen, such rows should be eliminated before the training.
		}

		#print STDERR "a_c[$j] = $a_c[$j]\n";
	}

	# Precompute an invariant matrix (see technical report):
	my @tmp1;
	for (my $i = 0; $i < $m; $i++) {
		my @row;
		for (my $j = 0; $j < $n; $j++) {
	...
	my $delta = 1;
	my $old_error = 0;
	while ($delta > 0.001 && $counter < $max_iterations) {
		# 2a. Compute averages ...
		#  for co-clusters:
		my @a_coc;
				for (my $g = 0; $g < $k; $g++) {
				...
				for (my $j = 0; $j < $n; $j++) {
					if ($confidence->[$i][$j]) {
						my $local_sum =
							...
							+ $a_rc[$g]
							+ $a_cc[$item_clustering[$j]];
						$sum = $sum + $local_sum * $local_sum;
					}
				}
				$values[$g] = $sum;
			}
			my $cluster = array_get_minimum_index(@values);
	...
	print STDERR "Clustering converged after $counter iterations.\n" if $verbose;
...
sub compute_global_average ($$) {
	...
	my $m = @$ratings;		# number of rows
	my $n = @{$ratings->[0]};	# number of columns

	my $sum = 0;
	my $number_of_ratings = 0;
	for (my $i = 0; $i < $m; $i++) {
		my @tmp;
		for (my $j = 0; $j < $n; $j++) {
			if ($confidence->[$i][$j] == 1) {
	...
	if ($number_of_ratings > 0) {
		...
		warn "Empty confidence matrix.\n";
		return 0;
		#return ($scale_from + $scale_to) / 2;
	}
}
